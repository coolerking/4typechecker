...
public class ServerUtils {
	 ...
	 */
	public static final String[][] JAPANESE_IP = {
		{"1.0.16.0", "20"},
		{"1.0.64.0", "18"},
		{"1.1.64.0", "18"},
		{"1.5.0.0", "16"},
	...
	/**
	 * IPアドレスが国内ドメインかどうかチェックする。
	 * ただしIPv6の場合はチェックせず真値を返す
	 * @param ip IPアドレス
	 * @return 真：国内orIPv6、偽：国外
	 */
	public static boolean isJapaneseIP(String ip){
		if(CRON_KICKER.equals(ip)) return true;
		if(ip==null) return false;
		if(ip.contains(":")) return true;
		for(int index=0;index<JAPANESE_IP.length;index++){
			String[] range = JAPANESE_IP[index];
			if(ServerUtils.isScorp(ip, range[0], range[1])){
				return true;
			}
		}
		return false;
	 ...
	 */
	public static boolean isScorp(String ipaddr, String segaddr, String len){
		int length = -1;
		...
		if(length==-1){
			return ipaddr.equals(segaddr);
		}else return ServerUtils.isScorp(ipaddr, segaddr, length);
	}
